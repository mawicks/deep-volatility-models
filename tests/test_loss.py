...
def test_multivariate_likelihood():
    """
    `    This only check that multivariate_mixture_log_likelihood can be called
        with arguments of consistent dimension within the correct range.
    """
...
def test_multivariate_likelihood_cases(x, log_p, mu, sigma_inv, expected):
    ...
    log_loss = loss_functions.univariate_mixture_log_likelihood(
        x.squeeze(1), log_p, mu.squeeze(2), sigma_inv.squeeze(3).squeeze(2)
    )
    assert float(log_loss) == pytest.approx(expected, EPS)
