...
\begin{tikzpicture}
    [block/.style ={rectangle,draw=red!50,fill=red!20,minimum size=4mm,align=center},
     node distance=1.10cm and 1.10cm,
    ...
    \node[tensor={\the\cellsize}{18pt}{orange}] (flat latent) [right=of latent] [label=below:$n_f \times 1$] [label=above:{\it ts-latent}] {}
        ...
        ;
    \node[tensor={\the\cellsize}{24pt}{green}] (embedding) [below=of flat latent] [label=below:$n_e \times 1$] {}
        ;
        
    \node (hidden1) at ($(flat latent)!0.5!(embedding)$) {}
        ;
    \node[circle,fill,inner sep=1pt, outer sep=0pt] (aggregate) [right=0.6cm of hidden1] {}
        ;
    ...
    \node (sigma) [right=3.125cm of flat latent]  {$\sigma$};
    \node (mu) [right=3.125cm of embedding]  {$\mu$};
